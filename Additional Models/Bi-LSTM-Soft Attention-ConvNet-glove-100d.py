...
from tensorflow.keras.layers import (Input, Embedding, Bidirectional, LSTM,
                                   Conv1D, Dense, Dropout, SpatialDropout1D,
                                   GlobalMaxPooling1D, Multiply, Lambda)
...
BATCH_SIZE = 128
EPOCHS = 30
OUTPUT_DIR = "sarcasm_outputs_bilstm_attention_conv"
...
def attention_block(inputs):
    ...
    attention = Dense(1, activation='tanh')(inputs)  # (batch_size, seq_len, 1)
    attention = Lambda(lambda x: tf.keras.activations.softmax(x, axis=1))(attention)
    
    # Apply attention weights and return sequence
    attended = Multiply()([inputs, attention])
    return attended  # Shape: (batch_size, seq_len, hidden_size)
...
def build_model(embedding_matrix):
    ...
    # Attention mechanism (returns sequence)
    attended = attention_block(lstm_out)
    
    # Convolutional layers
    conv1 = Conv1D(64, 3, activation='relu')(attended)
    conv1 = GlobalMaxPooling1D()(conv1)
    
    conv2 = Conv1D(64, 5, activation='relu')(attended)
