...
MAX_LEN = 60  # Increased sequence length for richer context
VOCAB_SIZE = 25000
EMBEDDING_DIM = 300
BATCH_SIZE = 128
EPOCHS = 30
...
# Optimized Bi-GRU Model with 300D embeddings
...
plt.title('Bi-GRU Accuracy Curves (GloVe 840B)', fontsize=12)
plt.ylabel('Accuracy')
plt.xlabel('Epoch')
plt.legend()
...
plt.title('Bi-GRU Loss Curves (GloVe 840B)', fontsize=12)
plt.ylabel('Loss')
plt.xlabel('Epoch')
plt.legend()

plt.savefig(os.path.join(OUTPUT_DIR, 'training_curves-bigru-glove-300d.png'), bbox_inches='tight', dpi=300)
plt.close()
...
with open(os.path.join(OUTPUT_DIR, 'classification_report-bigru-glove-300d.txt'), 'w') as f:
    f.write("Bi-GRU with GloVe 840B Classification Report:\n")
    f.write(report)
...
plt.title('Bi-GRU Confusion Matrix (GloVe 840B)', fontsize=12)
plt.ylabel('True Label')
plt.xlabel('Predicted Label')
plt.savefig(os.path.join(OUTPUT_DIR, 'confusion_matrix-bigru-glove-300d.png'), bbox_inches='tight', dpi=300)
plt.close()
